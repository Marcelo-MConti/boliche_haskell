...
saida_rodadas [(r1,r2)] _ = ""++r1++" "++r2++" | " --se recebo uma lista, com uma tupla
...
tuplaToArrayInt ("X", "_") = [10]
tuplaToArrayInt ("X", b) = [10, charToInt b]
...
pontuarFrames 10 ((a, b):(c, d):xs) 
    | b == "/" = charToInt c + 10
    | c == "/" = charToInt a + 10 
    | c /= "/" = charToInt a + charToInt b + charToInt c 
    | otherwise = error "aqui"
pontuarFrames frame ((a,b):xs)
    | frame >= 11 = 0 -- só tenho no maximo 10 frames
    | a == "10" && frame == 10 = 10 + sum bonus + charToInt b 
    | a == "10" = 10 + sum bonus + pontuarFrames (frame + 1) xs -- Tive um Strike
    | b == "/" = spare -- deu um spare
    | b /= "/" = charToInt a + charToInt b + pontuarFrames (frame + 1) xs
    | otherwise = error "hfjdkhdg"
    where
